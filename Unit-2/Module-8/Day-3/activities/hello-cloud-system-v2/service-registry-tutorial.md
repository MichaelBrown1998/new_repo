...
* The Random Greeting Service is a REST web service that serves up one of several canned greetings at random. Like the Hello Cloud Service, it uses Config Server for its configuration files. 
...
1. Create the Random Greeting Service
1. Modify the Hello Cloud Service to use the Random Greeting Service
...
1. Create the configuration file and check it into the Git repo
1. Configure our Random Greeting Service to use the Config Server
1. Write the Controller the serves up the greetings
1. Add annotations so that our service will register itself with the Eureka Service Registry
...
Although we are moving our application's configuration settings to the Config Server, we must have minimal configuration within the service itself. We do this through the ```application.properties``` file. This file contains a setting for our application name and the location of the Config Server. In the file ```src/main/resources/application.properties``` in your ```random-greeting-service``` project, add the following entries:
...
- The host and port we use for the ```spring.config.import``` must match the host and port values on which our Config Server is running (the Config Server was created in the previous tutorial).
...
This, in fact, requires no action at all. In the past, the main application class required the annotation `@EnableDiscoveryClient` in order for the application to register with Eureka, but now registration is automatic as long as the application includes the `spring-cloud-starter-netflix-eureka-client` dependency (see below).
...
1. Modify the ```hello-cloud-service.properties``` file.
1. Modify the Controller to call the Random Greeting Service.
...
1. We autowire a DiscoveryClient instance. We'll use this to contact Eureka and ask about the connection details of the Random Greeting Service. The version we will use is the `org.springframework.cloud...` version, rather than the `com.netflix...` version.
1. We include a RestTemplate property. This will used to communicate with the Random Greeting Service. It allows us to make REST calls from our Java code.
1. We use the ```@Value``` annotation to get the values of the randomGreetingServiceName, serviceProtocol, and servicePath properties in our config file.
1. ```helloCloud``` Method:
   1. We use the DiscoveryClient to ask for the Random Greeting Service by name.
   1. We combine the serviceProtocol and servicePath from our configuration file with the host and port of the Random Greeting Service from Eureka to create the URI for the Random Greeting Service.
   1. We use the restTemplate and the URI to call the Random Greeting Service and get our "hello" greeting.
...
1. random-greeting-service
1. hello-cloud-service
